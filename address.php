...
function create_email_id_mapping($data)
{
    $mapping = array();
    foreach ($data as $row) {
        $mapping[$row[1]] = $row[37]; // Assuming ID is in the first column (index 0) and email is in the second column (index 1)
    }
    return $mapping;
}
...
function find_matching_rows($data1, $data2, $email_id_mapping)
{
    ...
    foreach ($data2 as $row) {
        ...
        if (isset($email_id_mapping[$email])) {
            $row[37] = $email_id_mapping[$email]; // Replace the ID value with the one from the mapping
            $matching_rows[] = $row;
        }
    }
...
$output_file = 'address_matching_rows_ukeu_us.csv';
